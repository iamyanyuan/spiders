...
class FateadmApi():
    ...
    def Predict(self, pred_type, img_data, head_info=""):
        ...
        if rsp.ret_code == 0:
            ...
            LOG('识别结果：{}'.format(rsp.pred_rsp.value))
        else:
...
def TestFunc():
    pd_id = "126***"  # 用户中心页可以查询到pd信息
    pd_key = "8YYzMP****************"
    app_id = "326821"  # 开发者分成用的账号，在开发者中心可以查询到
    app_key = "CtGg/klcffDbp/FVMUwqF81vkhLrfudw"
    # 识别类型，
    # 具体类型可以查看官方网站的价格页选择具体的类型，不清楚类型的，可以咨询客服
    # pred_type = "40600"  # 6个汉字
    pred_type = "40400"  # 4个汉字
